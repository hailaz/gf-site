...
:::
:::tip
对于泛型版本的集合容器（如 `TSet[T]`），框架提供了 `NilChecker` 函数用于自定义“哪些元素应当被视为 nil”，以解决包含指针、接口等类型时的 typed nil 判定问题。懒加载或条件写入方法（如 `AddIfNotExist*` 系列）在真正写入前会调用该函数，当判定为 nil 时通常不会将该元素加入集合；未设置 `NilChecker` 时，行为与历史版本保持一致。
:::

## 泛型 NilChecker 相关方法

- **说明**：泛型版本的 `TSet[T]` 提供了带 `NilChecker` 的构造函数以及运行时注册方法，包括：
...
- **作用**：通过 `NilChecker` 自定义“哪些元素视为 nil”，懒加载/条件写入方法（如 `AddIfNotExist*`、`AddIfNotExistFunc*` 系列）在真正写入前会调用该函数，判定为 nil 时通常不会将该元素加入集合。
...
func ExampleTSet_NilChecker() {
    ...
    // 使用构造函数指定 NilChecker，将 *Student(nil) 视为“无效元素”，不会加入集合
