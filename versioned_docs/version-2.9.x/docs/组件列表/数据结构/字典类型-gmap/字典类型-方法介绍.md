...
:::
:::tip
对于泛型版本的字典容器（如 `KVMap[K, V]`、`ListKVMap[K, V]`），框架提供了 `NilChecker` 函数用于自定义“哪些值应当被视为 nil”，以解决包含指针、接口等类型时的 typed nil 判定问题。懒加载或条件写入方法（如 `GetOrSet*` 系列）在真正写入前会调用该函数，当判定为 nil 时通常不会写入该键值对；未设置 `NilChecker` 时，行为与历史版本保持一致。
:::

## 泛型 NilChecker 相关方法

- **说明**：泛型版本的 `KVMap[K, V]` 和 `ListKVMap[K, V]` 提供了带 `NilChecker` 的构造函数以及运行时注册方法，包括：
...
- **作用**：通过 `NilChecker` 自定义“哪些值视为 nil”，懒加载/条件写入方法（如 `GetOrSet*`、`GetOrSetFunc*` 系列）在真正写入前会调用该函数，判定为 nil 时通常不会写入该键值对。
...
func ExampleKVMap_NilChecker() {
    ...
    // 使用构造函数指定 NilChecker，将 *Student(nil) 视为“无值”，不会写入 map
    ...
    // 也可以先创建，再通过 RegisterNilChecker 注册判定逻辑
