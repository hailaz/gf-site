...
sidebar_position: 0
hide_title: true
keywords: [GoFrame,GoFrame框架,队列,gqueue,gtimer,Pop,Push,队列长度,队列关闭,glist,泛型队列,TQueue]
description: '使用GoFrame框架中的gqueue组件进行基本的队列操作，包括元素的入队和出队、队列长度的获取以及队列的关闭。v2.10版本新增泛型队列TQueue，提供类型安全的队列操作。详细演示了通过Push和Pop方法管理队列元素，并展示了队列与glist链表的关系，确保在GoFrame框架下高效构建并发安全的程序逻辑。'
---
