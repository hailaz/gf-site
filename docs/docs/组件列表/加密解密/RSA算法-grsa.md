...
title: 'RSA算法-grsa'
hide_title: true
keywords: [GoFrame,GoFrame框架,RSA算法,加密解密,grsa,公钥加密,私钥解密,数字签名,PKCS1,PKCS8,OAEP]
description: '在GoFrame框架中使用RSA算法进行加密解密操作。grsa组件提供完整的RSA加密支持，包括密钥对生成、公钥加密、私钥解密、多种填充模式（PKCS1、OAEP）和密钥格式（PEM、DER、Base64），适用于安全数据传输和数字签名场景。'
---
